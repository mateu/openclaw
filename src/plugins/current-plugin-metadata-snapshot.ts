...
export function withPluginMetadataSnapshotScope<T>(
  ...
  // Retained runtime generations already captured discovery. Re-entering their
  // scope must not resolve filesystem roots again, even without a fingerprint.
  const compatibleConfigFingerprints = options.trustConfigIdentity
    ? undefined
  ...
  const configFingerprint =
    options.config && !options.trustConfigIdentity
